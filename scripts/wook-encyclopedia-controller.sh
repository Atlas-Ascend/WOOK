...
set_chapter_gate() {
  local cid="$1" gate="$2" value="$3" tmp
  ensure_state
  tmp="$STATE.tmp"
...
c0_first_red() {
  local line
  line="$(WOOK_ROOT="$ROOT" bash scripts/wook-c0-golden-slice-controller.sh status | sed -n 's/^FIRST_RED_GATE=//p' | tail -n1)"
  [ -n "$line" ] || line="UNKNOWN"
  echo "$line"
}
...
first_red() {
  ...
  while IFS= read -r cid; do
    ...
    if [ "$status" != "QUALIFIED" ]; then
      if [ "$cid" = "C00" ] && [ -s scripts/wook-c0-golden-slice-controller.sh ]; then
        local inner
        inner="$(c0_first_red)"
        if [ "$inner" != "NONE" ]; then
          echo "C00:$inner"
...
cmd_audit() {
  ...
  local required=(
    ...
    scripts/wook-c0-golden-slice-controller.sh
  )
  ...
  bash -n scripts/wook-c0-golden-slice-controller.sh || fail=1
  if [ "$fail" -ne 0 ]; then
...
cmd_next() {
  ...
  if [ "$cid" = "C00" ]; then
    echo "NESTED_C00_TRUTH=BEGIN"
    WOOK_ROOT="$ROOT" bash scripts/wook-c0-golden-slice-controller.sh next
    echo "NESTED_C00_TRUTH=END"
    return 0
  fi
...
cmd_report() {
  sync_evidence >/dev/null || true
  ensure_state
  jq --arg red "$(first_red)" --arg root "$ROOT" '. + {first_red:$red,active_root:$root}' "$STATE"
}
