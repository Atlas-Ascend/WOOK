...
cmd_status(){ ensure_state; echo "======================================================================"; echo " WOOK C0 GOLDEN SLICE // STATUS"; echo "======================================================================"; for gate in "${GATES[@]}"; do printf '%-28s %s\n' "$gate" "$(jq -r --arg g "$gate" '.gates[$g]' "$STATE_FILE")"; done; echo "FIRST_RED_GATE=$(first_red_gate)"; }

cmd_next(){
  gate="$(first_red_gate)"; echo "PACKET=$PACKET"; echo "PHASE=C0_WHERE_ARE_MY_SHOES"; echo "FIRST_RED_GATE=$gate"
  case "$gate" in
    ...
    HANDSTAND_DAN) echo "NEXT_ACTION=bash scripts/resolve-gbstudio-runtime.sh && bash scripts/implement-c0-handstand-dan-001.sh" ;;
    CROCS_0_TO_2|COLLISION_TOPOLOGY) echo "NEXT_ACTION=bash scripts/resolve-gbstudio-runtime.sh && bash scripts/implement-c0-crocs-collision-001.sh" ;;
    HUD|PHONE|INVENTORY|QUEST_LOG) echo "NEXT_ACTION=bash scripts/implement-c0-hud-menu-001.sh"; echo "MISSION=Native contextual HUD and Start Select interface runtime" ;;
    GROUNDSCORE|RESPONSIBILITY|WOOK_KARMA) echo "NEXT_ACTION=bash scripts/implement-c0-state-mutations-001.sh"; echo "MISSION=Prove one-time Groundscore Responsibility and Wook Karma interactions" ;;
...
cmd_run(){
  cmd_sync >/dev/null; gate="$(first_red_gate)"; echo "RUN_FIRST_RED_GATE=$gate"
  case "$gate" in
    PAPA_WOOK_CONTROLLER|SNIFFANY|RACCOON_ENCOUNTER) bash scripts/run-character-golden-001.sh ;;
    HANDSTAND_DAN) bash scripts/resolve-gbstudio-runtime.sh; bash scripts/implement-c0-handstand-dan-001.sh ;;
    CROCS_0_TO_2|COLLISION_TOPOLOGY) bash scripts/resolve-gbstudio-runtime.sh; bash scripts/implement-c0-crocs-collision-001.sh ;;
    HUD|PHONE|INVENTORY|QUEST_LOG) bash scripts/implement-c0-hud-menu-001.sh ;;
    GROUNDSCORE|RESPONSIBILITY|WOOK_KARMA) bash scripts/implement-c0-state-mutations-001.sh ;;
...
cmd_audit(){
  ...
  required=(
    ...
    scripts/resolve-gbstudio-runtime.sh scripts/run-character-golden-001.sh
  ...
  python -m py_compile tools/build-c0-hud-menu.py tools/build-c0-state-mutations.py || fail=1
