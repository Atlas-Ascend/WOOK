...
cmd_status() {
  ...
  for gate in "${GATES[@]}"; do
    printf '%-28s %s\n' "$gate" "$(jq -r --arg g "$gate" '.gates[$g]' "$STATE_FILE")"
  done
...
cmd_next() {
  ...
  case "$gate" in
    ...
    HUD|PHONE|INVENTORY|QUEST_LOG)
      ...
      echo "MISSION=Native contextual HUD and Start Select interface runtime" ;;
    GROUNDSCORE|RESPONSIBILITY|WOOK_KARMA)
      ...
      echo "MISSION=Prove one-time Groundscore Responsibility and Wook Karma interactions" ;;
    SIDE_QUEST)
...
cmd_sync() {
  ...
  if [ -s "$R" ] && jq -e '.result=="WOOK_C0_HANDSTAND_DAN_NATIVE_PASS"' "$R" >/dev/null 2>&1; then
  ...
  R="$ROOT/docs/proof/receipts/C0-SIDE-QUEST-LATEST.json"
  if [ -s "$R" ] && jq -e '.result=="WOOK_C0_SIDE_QUEST_NATIVE_PASS"' "$R" >/dev/null 2>&1; then
    set_gate SIDE_QUEST PASS
